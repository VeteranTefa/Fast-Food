...
def afterReg(request):
    try :
        ...
        messages.info(request,"Duplicated Email or Phone Number")
        return render(request,"regestrationforrestuarnt.html")
...
def registerForCustomer(request):
    form=CreateUserForm()
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'User Creation Sucessfully for ' + user)
...
def registerForResturant(request):
    form=CreateUserForm2()
    if request.method == 'POST':
        form = CreateUserForm2(request.POST)
        if form.is_valid():

            form.save()
            user = form.cleaned_data.get('username')
            messages.success(request, 'User Creation Sucessfully for ' + user)
            return redirect('loginresturant')

        else:
            messages.info(request, "Invalid Username or password ")

    return render(request, 'resturantRegister.html', {'form': form})

def loginForResturant(request):
    if request.method == 'POST':
        username = request.POST['username']
        password = request.POST['password']
    ...
    return render(request, 'loginresturant.html', {})
